...
class HomeController extends Controller
{
    ...
    public function redirectRole(){
       ...
       $EiapayPending=Payment::where('payment_for','=','eia')->where('user_id','=',$userId)->where('status','=','In Process...')->get()->count();
        $EiapayDeclined=Payment::where('payment_for','=','eia')->where('user_id','=',$userId)->where('status','=','Declined')->get()->count();
        ...
        $IetspayPending=Payment::where('payment_for','=','iets')->where('user_id','=',$userId)->where('status','=','In Process...')->get()->count();
        $IetspayDeclined=Payment::where('payment_for','=','iets')->where('user_id','=',$userId)->where('status','=','Declined')->get()->count();
        ...
        $paymentApcs = Payment::with($join)
                ...
                //->where('type','=','PRE-QUALIFICATION REGISTRATION')
                ->where('user_id','=',$userId)->orderBy('id', 'desc')->get();
        $ApcspayApprove=Payment::where('payment_for','=','apcs')->where('user_id','=',$userId)->where('status','=','Approved')->get()->count();
        $ApcspayPending=Payment::where('payment_for','=','apcs')->where('user_id','=',$userId)->where('status','=','In Process...')->get()->count();
        $ApcspayDeclined=Payment::where('payment_for','=','apcs')->where('user_id','=',$userId)->where('status','=','Declined')->get()->count();
        ...
        $apcsConsultant = Cert::where('user_id','=',$userId)->where('category','=','apcs')->get();
         //return $paymentEia;
            ...
            $data = [
                 ...
                 'IetspayPending'=>$IetspayPending,
                 'IetspayDeclined'=>$IetspayDeclined,

                 'ApcspayApprove'=>$ApcspayApprove,
                 'ApcspayPending'=>$ApcspayPending,
                 'ApcspayDeclined'=>$ApcspayDeclined,

                 'EiapayApprove'=>$EiapayApprove,
                 'EiapayPending'=>$EiapayPending,
                 'EiapayDeclined'=>$EiapayDeclined,
            ];
