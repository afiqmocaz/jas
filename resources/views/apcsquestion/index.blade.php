			...
			<div class="navbar templatemo-nav" id="navbar">
      		...
      		<div class="navbar-collapse collapse">
					...
					<li><a href="{{ url('index/create') }}">HOME</a></li>
	                  <li  class="active"><a href="{{ url('announce/show') }}">ANNOUNCEMENT</a></li>
	      	...
	      	<div class="breadcrumb flat">
    			...
    			<div style="width: 100%;" class="panel-group" >
				    <div class="panel panel-primary" style="border-color:#4CAF50;">
				     ...
				     <div id="collapse1" class="panel-collapse collapse">
				        ...
				        •  Secretariat APCS can enter question,Answer (A,B,C,D) and correct answer at the text field provided.<br>
						•  Secretariat APCS can select Type of Question and Level of Question at the radio button provided.<br>
						•  Secretariat APCS can click button "Save" to save the question of comprehensive examination.<br>
						•  Secretariat APCS can view the previous filled question in the table below.<br>
						•  Secretariat APCS can click "Edit" button to edit the question.<br>
						•  Secretariat APCS can click "Edit Option" button to edit the question's option.<br>
						•  Secretariat APCS can click "X" button to delete the question.<br>
						•  Secretariat APCS can click button "Save" to save the question of Comprehensive Exam.
				        </div>
				      </div>
				    </div>
				  </div>
				...
				<br>
				  ...
				  <div class="modal2-content">
				    ...
				    <div class="modal2-body">
				      <nav>
					      	<iframe src="{{ url('specialized') }}" width="200" height="450" scrolling="yes">
  						<p>Your browser does not support iframes.</p>
						</iframe></nav>

						<article>
						<iframe src="{{ url('related') }}" width="200" height="450" scrolling="yes">
  						<p>Your browser does not support iframes.</p>
						</iframe></article>
					      	</div><br>
				  ...
				  <div class="modal1-content">
				    ...
				    <div class="modal1-body">
				      {!! Form::open(array('url'=>'apcsquestion/uploadFiles', 'data-parsley-validate' => '', 'method'=>'POST', 'enctype' => 'multipart/form-data', 'files'=>true)) !!}
					    ...
					    <div class="form-group">
					      ...
					      <div class="input-group" style="width: 100%;">
				        	...
				        	<div class="input-group" style="width: 100%;">
						    <span class="input-group-addon" style="width: 40%;" id="specialized" name="specialized_id">Specialized Area</span>
						    <select id="specialized" name="specialized_id" class="form-control" required="" style="width:97.7%">
				        	<option value="0" disabled="true" selected="true">--Please Select--</option>

				        	@foreach($specializes as $cat)
				        	<option value="{!! $cat->id !!}">{!! $cat->specialized !!}</option>
				        	@endforeach
				        	</select>
						    
				        	</nav>

							<br><br><br><div class="input-group" style="width: 100%;">
				        	...
				        	<div class="input-group" style="width: 100%;">
						    <span class="input-group-addon" style="width: 40%;" id="related" name="related_id">Related Reference</span>
						    <select id="related" name="related_id" class="form-control" required="" style="width:97.7%">
				        	<option value="0" disabled="true" selected="true">--Please Select--</option>

				        	@foreach($relates as $catr)
				        	<option value="{!! $catr->id !!}">{!! $catr->related !!}</option>
				        	@endforeach
				        	</select>
				...
				<nav>
				<button id="myBtn" style="margin-left:15px;" class="btn btn-success" >Add New Question</button>
				...
				<table class="order-table table" id="list" style="width:90%;" align="center">
				      ...
				      <tr style="background-color: #4CAF50; color: white;">
				        <th style="width:3%;">No.</th>
				         <th style="width:10%">Specialized Area</th>
				      ...
				      <tbody>
						@foreach($question as $apcsquestions)
				      <tr>
				        <td rowspan="4"></td>
				         <td rowspan="4">{{$apcsquestions->specialized->specialized}}</td>
				       <td rowspan="4">
				       	@if (count($apcsquestions->original_filename) === 1)
				       <img class="file" src="/uploads/{{$apcsquestions->original_filename}}" width="100px"></img><a class="file" href="/uploads/{{$apcsquestions->original_filename}}"><br><center>{{$apcsquestions->limg}}</center></img>
				       @else
				       @endif
				       </td>
				        <td rowspan="4">{{$apcsquestions->question}}</td>
				        <td>I. {{$apcsquestions->i}} <br> ii. {{$apcsquestions->ii}} <br> iii. {{$apcsquestions->iii}} <br> iv. {{$apcsquestions->iv}}</td>
				     
				          @foreach($apcsquestions->options as $option)
                        <br>
                        <td rowspan="4" class="radio-inline">
                            <ol
                                type="1"
                                name="answers[{{ $apcsquestions->id }}]"
                                value="{{ $option->id }}">
                            {{ $option->option }}</ol>
                        </td>
                    @endforeach
				    ...
				    @endif   
				    @endforeach    
				        <td rowspan="4">{{$apcsquestions->related}}</td>
				        <td rowspan="4">{{$apcsquestions->level}}</td>
				        <td rowspan="4" id="but" align="right"><button>Approve<br>Question</button>
				        <br><br><button type="button" style="width:200%;" title="Question Preview" data-toggle="modal" data-target="#yourModal{{$apcsquestions->id}}">Preview</button>
				        </td>

				        <td><center><a href="{{ route('apcsexamoption.index') }}" >
				        	{{ HTML::image('images/edit.png', 'a Edit', array( 'width' => '20', 'title' => 'Edit Options')) }}</a> </center><br></td>
				        <td><center><a href="{{ route('apcsquestion.edit', $apcsquestions->id) }}" >
				        	{{ HTML::image('images/edit.png', 'a Edit', array( 'width' => '20', 'title' => 'Edit')) }}</a> </center><br></td>

				        	<td>{!! Form::open(['route' => ['apcsquestion.destroy', $apcsquestions->id], 'method' => 'DELETE', 'onsubmit' => 'return ConfirmDelete()']) !!}

				        	{!! Form::image('images/delete.ico', 'a Delete', ['type' => 'submit',  'width' => '17', 'title' => 'Delete'] ) !!}

				        	{!! Form::close() !!}
				        	</td>
				        <td rowspan="4" id="by" rowspan="4">Aizat Hamdan Jailani</td>
				      </tr>
				      ...
				      @foreach($question as $apcsquestions)    
				    <div class="modal fade" id="yourModal{{$apcsquestions->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
				      <div class="modal-dialog" role="document">
				        <div class="modal-content">
				          ...
				          @if (count($apcsquestions->original_filename) === 1)
				          <center><img class="file" src="/uploads/{{$apcsquestions->original_filename}}" width="100px"></img><a class="file" href="/uploads/{{$apcsquestions->original_filename}}"></center>
				          <br><center>{{$apcsquestions->limg}}</center></a><br>
				          @else   
						  @endif

				          

				            {{$apcsquestions->id}}. {{$apcsquestions->question}}

				           	@if (count($apcsquestions->i) === 1)
				            <br><br>I. {{$apcsquestions->i}}
				            @else   
							@endif

							@if (count($apcsquestions->ii) === 1)
				            <br>II. {{$apcsquestions->ii}}
				            @else   
							@endif

							@if (count($apcsquestions->iii) === 1)
				            <br>III. {{$apcsquestions->iii}}
				            @else   
							@endif

							@if (count($apcsquestions->iv) === 1)
				            <br>IV. {{$apcsquestions->iv}}
				            @else   
							@endif
