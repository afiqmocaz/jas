...
<html>
 ...
 <!-- <h4 class="text-center">SECTION A: PERSONAL INFORMATION</h4>
  ...
  <div class="section" style="padding: 0px;">
    ...
    $no=1;?>
    <tbody>
    @foreach($ietsSectionB as $ietsSectionBs)
      <tr  style="border-bottom: 1px solid #ddd;background-color:">
       <td>{{ $no++ }}</td>
        <td>{{ $ietsSectionBs->coursetitle }}</td>
        <td>{{ $ietsSectionBs->major }}</td>
        <td>{{ $ietsSectionBs->universityname }}</td>
        <td>{{ date('d/m/Y', strtotime($ietsSectionBs->from)) }}</td>
        <td>{{ date('d/m/Y', strtotime($ietsSectionBs->to)) }}</td>
        <td>
        <a class="file" href="/uploads/{{$ietsSectionBs->cert}}" target="_blank" style="">{{$ietsSectionBs->cert}}</a>
        </td>
        <td class="{{($count>0)?'':'hide'}} "><center>
        <a href="{{ route('ietsSectionB.edit', $ietsSectionBs->id) }}" >
          {{ Form::image('/image/edit.png', 'a Edit', array('class' => 'btnimg', 'title' => 'Edit')) }}</a>
          </td>
          <td class="{{($count>0)?'':'hide'}} ">
          {!! Form::open(['route' => ['ietsSectionB.destroy', $ietsSectionBs->id], 'method' => 'DELETE', 'onsubmit' => 'return ConfirmDelete()']) !!}
  ...
  <div class="row">
            <div class="col-md-11" style=" margin-top: 10px; margin-bottom: 10px;" align="right">
              ...
              <a class="btn btn-primary" href="/ietsSectionA/create" role="button">Back</a>
              <a  data-toggle="modal" data-target="#ietsSecB" >
            {{ Form::button('Add New', array('class'=>'btn btn-primary ', 'type'=>'submit')) }}</a>
              <a href="/ietsSectionC/create" >
...
<div class="section">
  ...
  <div class="row" >
   ...
   {!! Form::open(['route' => 'ietsSectionB.store', 'data-parsley-validate'=> '', 'files'=> true,'id'=>'myform']) !!}
            
		 @if(count($ietsSectionB) > 0)
	...
	<br>
        ...
        PhD
            ...
            <div class="input-group" >
             ...
             <input type="date" class="textbox" id="from" name="from"   />
            </div>
          </div>
          <div class="col-xs-5">
            <div class="input-group">
              ...
              <!-- {!! Form::text('to', '', array('id' => 'datepicker2', 'class' => 'form-control', 'style' => 'width: 70%')) !!} -->
               <input type="date" class="textbox" id="to" name="to" onchange="validateForm(this);" />
            </div>
          </div>   
           <br><br>
            </div>
...
$(document).ready(function() {
  $("#ietsSecB").on("hidden.bs.modal", function() {
