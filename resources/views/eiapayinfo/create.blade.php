			...
			<div class="navbar templatemo-nav" id="navbar">
	      	...
	      	<div class="breadcrumb flat">
				...
				<table class="table">
				    ...
				    @foreach($payinfo as $payinfos)
				      <tr>
				        <td>{{$users->name}}</td>
				        <td>{{$users->nric}}</td>
				        <td><center>{{$payinfos->amount}}</center></td>
				        <td><center>{{date('d/m/Y', strtotime($payinfos->date))}}</center></td>
				        <td><center>{{$payinfos->referenceNo}}</center></td>
				        <td><center><a style="color: blue;" href="/backend/uploads/{{$payinfos->slip}}">{{$payinfos->slip}}</a></center></td>
				        <td><center><form action="{{ url('payment/show') }}" method="get">
				        <button type="submit" style="width:80px;" name="pay" value="Approved"/>Approve</button><br><br>
					    <button type="submit" style="width:80px;" name="pay" value="Declined"/>Decline</button>
					</form></td>
				      </tr>
				      @endforeach
				    @endforeach
