...
@section('content')
@section('header', ': APCS : SECTION D - PRACTICAL EXPERIENCES')
<html>
  <head>
...
<!--                                <th style="width: 5%">Number of Months</th>-->
                        ...
                        $no=1;?>
                        <tbody>
                            @foreach($apcsSectionD as $apcsSectionDs)
                            <tr style="border-bottom: 1px solid #ddd;background-color: ">
                              <td>{{ $no++ }}</td>
                                <td>{{ date('d/m/Y', strtotime($apcsSectionDs->projectStart)) }}</td>
                                <td>{{ date('d/m/Y', strtotime($apcsSectionDs->projectComplete)) }}</td>
                                <td>{{ $apcsSectionDs->numdays }}</td>
<!--                                <td>{{ $apcsSectionDs->numMonths }}</td>-->
                                <td>{{ $apcsSectionDs->proposaltitle }}</td>
                                <td>{{ $apcsSectionDs->clientname }}</td>
                                <td>{{ $apcsSectionDs->clientaddress }}</td>
                                <td><a class="file" style="color:blue" href="/uploads/{{$apcsSectionDs->supportdoc}}" target="_blank">{{$apcsSectionDs->supportdoc}}</a>


                                </td>
                                <td ><center>

                            <a href="{{ route('apcsSectionD.edit', $apcsSectionDs->id) }}" >
                                {{ Form::image('/image/edit.png', 'a Edit', array('title' => 'Edit', 'class' => 'btnimg')) }}</a>
                            </td>
                            <td >
                                {!! Form::open(['route' => ['apcsSectionD.destroy', $apcsSectionDs->id], 'method' => 'DELETE', 'onsubmit' => 'return ConfirmDelete()']) !!}
                        ...
                        <tr style="border-bottom: 1px solid #ddd;background-color: #f2f2f2">
                            ...
                            <td><strong style="color: blue">{{number_format($apcsSectionD2,0)}}</strong></td>
                            <td colspan="6"></td>
                        </tr>
                        </tbody>
                    </table>
  ...
  <div class="row">
            <div class="col-md-11" style=" margin-top: 10px; margin-bottom: 10px;" <?=(($apcsSectionD2 >= 36) ? 'align="right"' : '' )?>>
               @if ($apcsSectionD2 >= 36)
                 <a class="btn btn-primary" href="/apcsSectionB/create" role="button">Back</a>
                 <a  data-toggle="modal" data-target="#apcsSecD" >{{ Form::button('Add New', array('class'=>'btn btn-primary', 'type'=>'submit')) }}</a>
                 <a href="/apcsSectionE/create" class="btn btn-primary" role="button">Next</a>
                @else
          ...
          <div class="modal fade" id="apcsSecD" role="dialog">
...
<div class="section">
  ...
  <div class="row" >
          ...
          {!! Form::open(['route' => 'apcsSectionD.store', 'data-parsley-validate'=> '', 'files'=> true,'id'=>'applicantFormC']) !!}
...
$(document).ready(function() {
  $("#apcsSecD").on("hidden.bs.modal", function() {
...
$("#datepicker2").change(function () {
    ...
    if ((Date.parse(endDate) <= Date.parse(startDate))) {
       ...
       $('[name="datepicker2"]').val('');
          $('#applicantFormC').bootstrapValidator('revalidateField', 'datepicker2', false);
    }
});
