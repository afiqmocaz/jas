		...
		<div>
			...
			<div class="navbar templatemo-nav" id="navbar">
	      	...
	      	<div class="breadcrumb flat">
				    ...
				    <a href="{{ url('schedule/iets') }}">Schedule</a>
				    <a class="active">Exam Attendant List</a>        
				  </div>
				</div>
				...
				<div style="width: 100%;" class="panel-group" >
				    <div class="panel panel-primary" style="border-color:#4CAF50;">
				      ...
				      <div id="collapse1" class="panel-collapse collapse">
						...
						•  Secretariat IETS can select the applicant attendant to take the attendant of applicant which is attend or absent.<br> 
						•  Secretariat IETS can select the Status to give the marks status of applicant which is pass,fail or acknowledge if the applicant is absent the examination.<br>
						•  Secretariat IETS can click button "Save" to save the applicant exam information that has been changes.
				        </div>
				      </div>
				    </div>
				  </div>
    			...
    			<table class="table">
				    <thead valign="top">
				      ...
				      <tbody>
				      @foreach($ietsexam as $ietsexams)
				      <tr>
				        <td></td>
				        <td>{{$ietsexams->applicantname}}</td>
				        <td>{{$ietsexams->IC}}</td>
				      
				      <td>{{$ietsexams->seat}}</td>
				      <td>{{$ietsexams->mark}}</td>
				      <td>{{$ietsexams->status}}</td>
				      </tr>
				      @endforeach
				      </tbody>
				  </table>  <br>
