...
<div class="section" style="padding: 0px;">
    <div class="container">
				  ...
				  <form action="{{ url('apcsguideline/create') }}" method="get">
...
<!--				<input type="search" class="light-table-filter" data-table="order-table"  placeholder="Search..." style="width: 20%; float:right;"><br><br>-->
				...
				<table class="table table-bordered" id="myTable" style="width: 95%">
				    ...
				    @foreach($apcsguideline as $apcsguideline)
				      <tr>
				        <td>{{$count++}}</td>
				        <td>{{$apcsguideline->subject}}</td>
				    
				        <td><a class="file" href="/guideline/{{$apcsguideline->original_filename}}" target="_blank">{{$apcsguideline->original_filename}}</a></td>
				        <td id="but">
				        	<center><a href="{{ route('apcsguideline.edit', $apcsguideline->id) }}" >{!! Form::image('images/edit.png', 'a Delete', ['type' => 'submit',  'class' => 'btnimg', 'title' => 'Delete'] ) !!}</center>
