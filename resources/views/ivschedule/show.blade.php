		...
		<div>
			...
			<div class="navbar templatemo-nav" id="navbar">
	      	...
	      	<div class="breadcrumb flat">
				    ...
				    <a href="#">MANAGE</a>
				    <a href="{{ url('interview/apcs') }}">Profesional Interview</a>
				  ...
				  <div class="modal-content">
				    ...
				    <div class="modal-body">
				      {!! Form::open(array('route' => 'ivschedule.store', 'data-parsley-validate' => '')) !!}
						    ...
						    <select id="venue" name="venue_id" class="form-control" onchange='ChangeVal(this.value);' required="" style="width:97.7%">
				...
				<input type="search" class="light-table-filter" data-table="order-table"  placeholder="Search..." class="form-control" style="width: 17%; float:right; margin-right:15px;"><br><br>
				...
				<table class="order-table table" id="list" style="width:80%;" align="center">
				    <thead valign="top">
				      ...
				      <tr>
				        <td></td>
				        <td>{{$ivschedules->title}}</td>
				        <td>{{ date('d/m/Y', strtotime($ivschedules->date))}}</td>
				        <td>{{ date('h:i A', strtotime($ivschedules->time))}}</td>
				        <td>{{$ivschedules->venue->venue}}<br>{{$ivschedules->address}}<br>{{$ivschedules->state}}</td>
				        <td id="but" align="right">
				        	<!-- <center>{!! HTML::linkRoute('ivschedule.edit', 'Edit', array($ivschedules->id), array('class' => 'btn btn-primary btn-block')) !!}</center><br> -->
				        	<a href="{{ route('ivschedule.edit', $ivschedules->id) }}" >
				        	{{ HTML::image('images/edit.png', 'a Edit', array('class' => 'img-responsive cleaner', 'width' => '20', 'title' => 'Edit')) }}</a> </center><br></td>

				        	<td>{!! Form::open(['route' => ['ivschedule.destroy', $ivschedules->id], 'method' => 'DELETE', 'onsubmit' => 'return ConfirmDelete()']) !!}
