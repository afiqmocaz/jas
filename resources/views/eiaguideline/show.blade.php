...
<div class="section" style="padding: 0px;">
    <div class="container">
				  ...
				  <form action="{{ url('eiaguideline/create') }}" method="get">
					    <input type="submit" class="btn btn-primary" value="Add Guidelines"/>
					</form>
...
<!--				<input type="search" class="light-table-filter" data-table="order-table"  placeholder="Search..." style="width: 20%; float:right;"><br><br>-->
				...
				<table class="table table-bordered" id="myTable" style="width: 95%">
				      ...
				      <tr>
				        ...
				        <th onclick="sortTable(1)" style="width: 15%">Subject</th>
				      
				        <th onclick="sortTable(3)" style="width: 25%">Uploaded File</th>
				        <th colspan="2" onclick="sortTable(4)" style="width: 5%"><center>Action</center></th>
				      </tr></thead>
				    
				    <tbody>
                                    <div class="hide">{{$count = 1}}</div>
				    @foreach($eiaguideline as $eiaguideline)
				      <tr>
				        <td>{{$count++}}</td>
				        <td>{{$eiaguideline->subject}}</td>
				    
				        <td><a class="file" href="/guideline/{{$eiaguideline->original_filename}}" target="_blank">{{$eiaguideline->original_filename}}</a></td>
				        <td id="but">
				        	<center><a href="{{ route('eiaguideline.edit', $eiaguideline->id) }}" >


                                                {!! Form::image('images/edit.png', 'a Delete', ['type' => 'submit',  'class' => 'btnimg', 'title' => 'Edit'] ) !!}</center>
