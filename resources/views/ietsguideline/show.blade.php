...
<!--				<input type="search" class="light-table-filter" data-table="order-table"  placeholder="Search..." style="width: 20%; float:right;"><br><br>-->
				...
				<table class="table table-bordered" id="myTable" style="width: 95%">
				    ...
				    @foreach($ietsguideline as $ietsguideline)
				      <tr>
				        <td>{{$count++}}</td>
				        <td>{{$ietsguideline->subject}}</td>
				    
				        <td><a class="file" href="/guideline/{{$ietsguideline->original_filename}}" target="_blank">{{$ietsguideline->original_filename}}</a></td>
				        <td id="but">
				        	<center><a href="{{ route('ietsguideline.edit', $ietsguideline->id) }}" >
                                                {!! Form::image('images/edit.png', 'a Delete', ['type' => 'submit',  'class' => 'btnimg', 'title' => 'Delete'] ) !!}</center>
