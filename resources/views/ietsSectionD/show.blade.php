...
<html>
 ...
 <!-- <h4 class="text-center">SECTION A: PERSONAL INFORMATION</h4>
  ...
  $no=1;?>
    <tbody>
      @foreach($ietsSectionD as $ietsSectionDs)
      <tr style="border-bottom: 1px solid #ddd;background-color: ">
      <td>{{$no++ }}</td>
        <td>{{ date('d/m/Y', strtotime($ietsSectionDs->starttrainning)) }}</td>
        <td>{{ date('d/m/Y', strtotime($ietsSectionDs->endtraining)) }}</td>
        <td>{{ $ietsSectionDs->seminarname }}</td>
        <td>{{ $ietsSectionDs->venue }}</td>
        <td><center>
        <a href="{{ route('ietsSectionD.edit', $ietsSectionDs->id) }}" >
          {{ Form::image('/image/edit.png', 'a Edit', array('title' => 'Edit', 'class' => 'btnimg')) }}</a>
          </td>
          <td>
          {!! Form::open(['route' => ['ietsSectionD.destroy', $ietsSectionDs->id], 'method' => 'DELETE', 'onsubmit' => 'return ConfirmDelete()']) !!}
      ...
      @endforeach
            ...
            <div class="col-md-11" style=" margin-top: 10px; margin-bottom: 10px;" align="right">
              ...
              <a class="btn btn-primary" href="/ietsSectionC/create" role="button">Back</a>
              <a data-toggle="modal" data-target="#ietsSecD">{{ Form::button('Add New', array('class'=>'btn btn-primary ', 'type'=>'submit')) }}</a>
              <a href="/ietsSectionE/create" class="btn btn-primary" role="button">Next</a>
            ...
            <!-- add content -->
  <div class="modal fade" id="ietsSecD" role="dialog">
...
<div class="section">
  ...
  <div class="row" >
    ...
    {!! Form::open(['route' => 'ietsSectionD.store', 'data-parsley-validate'=> '', 'files'=> true,'id'=>'applicantFormD']) !!}
...
$(document).ready(function() {
  $("#ietsSecD").on("hidden.bs.modal", function() {
